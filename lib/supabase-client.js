...
async function deleteRegistrationRequest(requestId) {
  const client = await getSupabaseClient()
  if (!client) throw new Error("Supabase not available")
...
async function getClasses() {
  console.log("[v0] getClasses: Fetching all classes...")
  const client = await getSupabaseClient()
  if (!client) throw new Error("Supabase not available")
  ...
  if (error) {
    console.error("[v0] getClasses: Error fetching classes:", error)
    throw error
  }

  console.log("[v0] getClasses: Successfully fetched", data?.length || 0, "classes")
  return data || []
}

async function createClass(className) {
  console.log("[v0] createClass: Creating class:", className)
  const client = await getSupabaseClient()
  if (!client) throw new Error("Supabase not available")

  const currentUser = await getCurrentUser()
  if (!currentUser || currentUser.role !== "admin") {
    throw new Error("Vous devez être administrateur pour créer une classe")
  }
  ...
  if (error) {
    console.error("[v0] createClass: Error creating class:", error)
    throw error
  }

  console.log("[v0] createClass: Class created successfully:", data.id)
  return data
}

async function deleteClass(classId) {
  console.log("[v0] deleteClass: Deleting class:", classId)
  const client = await getSupabaseClient()
  if (!client) throw new Error("Supabase not available")

  const currentUser = await getCurrentUser()
  if (!currentUser || currentUser.role !== "admin") {
    throw new Error("Vous devez être administrateur pour supprimer une classe")
  }

  const { error } = await client.from("classes").delete().eq("id", classId)

  if (error) {
    console.error("[v0] deleteClass: Error deleting class:", error)
    throw error
  }

  console.log("[v0] deleteClass: Class deleted successfully")
}

async function updateClass(classId, newName) {
  console.log("[v0] updateClass: Updating class:", classId, "with name:", newName)
  const client = await getSupabaseClient()
  if (!client) throw new Error("Supabase not available")

  const currentUser = await getCurrentUser()
  if (!currentUser || currentUser.role !== "admin") {
    throw new Error("Vous devez être administrateur pour modifier une classe")
  }
  ...
  if (error) {
    console.error("[v0] updateClass: Error updating class:", error)
    throw error
  }

  console.log("[v0] updateClass: Class updated successfully:", data.id)
  return data
}

async function getClassMembers(classId) {
  console.log("[v0] getClassMembers: Fetching members for class via API:", classId)

  try {
    // Call the Vercel API function instead of using Supabase client directly
    // This bypasses PostgREST's relationship embedding completely
    ...
    if (!response.ok) {
      ...
      console.error("[v0] getClassMembers: API error:", error)
      throw new Error(error.error || "Failed to fetch members")
    }

    const { members } = await response.json()
    console.log("[v0] getClassMembers: Fetched", members?.length || 0, "members from API")
    return members || []
  } catch (err) {
    console.error("[v0] getClassMembers: Exception:", err)
    throw err
  }
}

async function addStudentToClass(classId, userId) {
  console.log("[v0] addStudentToClass: Adding student", userId, "to class", classId)
  const client = await getSupabaseClient()
  if (!client) throw new Error("Supabase not available")

  const currentUser = await getCurrentUser()
  if (!currentUser || currentUser.role !== "admin") {
    throw new Error("Vous devez être administrateur pour ajouter un étudiant à une classe")
  }
  ...
  if (error) {
    console.error("[v0] addStudentToClass: Error adding student:", error)
    if (error.code === "23505") {
      throw new Error("Cet étudiant est déjà membre de cette classe")
    }
    throw error
  }

  console.log("[v0] addStudentToClass: Student added successfully")
  return data
}

async function removeStudentFromClass(classId, userId) {
  console.log("[v0] removeStudentFromClass: Removing student", userId, "from class", classId)
  const client = await getSupabaseClient()
  if (!client) throw new Error("Supabase not available")

  const currentUser = await getCurrentUser()
  if (!currentUser || currentUser.role !== "admin") {
    throw new Error("Vous devez être administrateur pour supprimer un étudiant d'une classe")
  }
  ...
  if (error) {
    console.error("[v0] removeStudentFromClass: Error removing student:", error)
    throw error
  }

  console.log("[v0] removeStudentFromClass: Student removed successfully")
}

async function getUserClasses(userId) {
  console.log("[v0] getUserClasses: Fetching classes for user:", userId)
  const client = await getSupabaseClient()
  if (!client) throw new Error("Supabase not available")
  ...
  if (error) {
    console.error("[v0] getUserClasses: Error fetching user classes:", error)
    throw error
  }

  console.log("[v0] getUserClasses: User is member of", data?.length || 0, "classes")
  return data || []
}
