...
async function approveRegistration(requestId) {
  ...
  console.log("[v0] Starting approval process for request:", requestId)
  ...
  if (fetchError) {
    console.error("[v0] Error fetching registration request:", fetchError)
    throw fetchError
  }

  console.log("[v0] Registration request data:", request)

  const { data: existingUser } = await client.from("users").select("email").eq("email", request.email).single()

  if (existingUser) {
    console.log("[v0] User already exists, just updating request status")
    // User already exists, just update the request status
    ...
    return { request, user: existingUser }
  }

  // Create the user account
  console.log("[v0] Creating user account for:", request.email)
  ...
  if (createError) {
    console.error("[v0] Error creating user account:", createError)
    throw new Error(`Failed to create user account: ${createError.message}`)
  }

  console.log("[v0] User account created successfully:", newUser)

  // Update the request status only after successful user creation
  const { error: updateError } = await client
    .from("registration_requests")
    .update({
      status: "approved",
