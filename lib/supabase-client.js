...
async function trackDeviceLogin(userZalo, deviceFingerprint, deviceInfo) {
  ...
  try {
    ...
    if (existingDevice) {
      ...
      const { error: insertError } = await client.from("device_sessions").insert({
        ...
        login_count: 1,
        is_locked: true,
        locked_until: new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString(), // 7 days from now
      })

      if (insertError) throw insertError
    }
  } catch (error) {
    console.error("[v0] Device tracking error:", error)
    throw error
  }
}

async function checkDeviceLimit(userZalo) {
  const client = await getSupabaseClient()
  if (!client) throw new Error("Base de données non disponible")

  const { data, error } = await client.from("device_sessions").select("*").eq("user_zalo", userZalo)
  ...
  return {
    ...
    isLimitReached: (data?.length || 0) >= MAX_DEVICES,
  }
}

async function getUserDevices(userZalo) {
  const client = await getSupabaseClient()
  if (!client) throw new Error("Base de données non disponible")
...
async function removeDevice(deviceId, userZalo) {
  const client = await getSupabaseClient()
  if (!client) throw new Error("Base de données non disponible")
  ...
  const lockedUntil = new Date(device.locked_until)
  const now = new Date()

  if (device.is_locked && lockedUntil > now) {
    const daysRemaining = Math.ceil((lockedUntil - now) / (1000 * 60 * 60 * 24))
    throw new Error(
      `Cet appareil est verrouillé pour ${daysRemaining} jour(s) supplémentaire(s). Vous pourrez le supprimer le ${lockedUntil.toLocaleDateString("fr-FR")}.`,
    )
  }
...
async function loginWithEmailAndPin(email, pin) {
  ...
  if (user.role === "student" && window.DeviceFingerprint) {
    try {
      const { fingerprint, deviceInfo } = await window.DeviceFingerprint.generate()

      // Check device limit before allowing login
      const deviceCheck = await checkDeviceLimit(user.zalo)

      // Get current device status
      const devices = await getUserDevices(user.zalo)
      ...
      await trackDeviceLogin(user.zalo, fingerprint, deviceInfo)
    } catch (deviceError) {
