...
async function getClassMembers(classId) {
  ...
  try {
    // Call the RPC function that returns JSON with embedded user data
    // This completely bypasses Supabase's relationship embedding system
    const { data, error } = await client.rpc('get_class_members_json', {
      p_class_id: classId
    })
    ...
    let members = []
    if (data) {
      members = Array.isArray(data) ? data : (data ? JSON.parse(JSON.stringify(data)) : [])
    }
