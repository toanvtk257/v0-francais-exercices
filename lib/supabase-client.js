...
async function waitForConfig() {
  ...
  console.error("Timeout waiting for config to load")
  return false
}
...
async function getSupabaseClient() {
  ...
  if (typeof window.supabase === "undefined" || !window.supabase.createClient) {
    console.error("Supabase CDN not loaded. Make sure to include the CDN script.")
    return null
  }
  ...
  if (!config || !config.url || !config.anonKey) {
    console.error("Missing Supabase configuration. Make sure lib/config.js is loaded.")
    return null
  }
...
async function trackDeviceLogin(userId, deviceFingerprint, deviceInfo) {
  ...
  if (!client) {
    console.error("trackDeviceLogin: Supabase client not available")
  ...
  try {
    ...
    console.log("trackDeviceLogin: Generated device name:", deviceName)
    console.log("trackDeviceLogin: UserAgent:", deviceInfo.userAgent)

    console.log("trackDeviceLogin: Checking for devices older than 7 days...")
    ...
    if (oldDevices && oldDevices.length > 0) {
      console.log("trackDeviceLogin: Found old devices, deleting them for weekly reset...")
      await client.from("device_sessions").delete().eq("user_id", userId).lt("first_seen", sevenDaysAgo.toISOString())
      console.log("trackDeviceLogin: Old devices deleted successfully")
    }

    console.log("trackDeviceLogin: Checking for existing device...")
    ...
    if (checkError && checkError.code !== "PGRST116") {
      console.error("trackDeviceLogin: Check error:", checkError)
      throw checkError
    }

    console.log("trackDeviceLogin: Existing device check result:", { existingDevice, hasError: !!checkError })

    if (existingDevice) {
      console.log("trackDeviceLogin: Updating existing device...")
      ...
      if (updateError) {
        console.error("trackDeviceLogin: Update error:", updateError)
        throw updateError
      }
      console.log("trackDeviceLogin: Device updated successfully")
    } else {
      const { data: userData } = await client.from("users").select("role").eq("id", userId).single()

      const isAdmin = userData?.role === "admin"
      console.log("trackDeviceLogin: User role check:", { isAdmin, role: userData?.role })
      ...
      if (!isAdmin) {
        ...
        console.log("trackDeviceLogin: Current device count:", deviceCount)

        if (deviceCount >= 2) {
          throw new Error("Limite de 2 appareils atteinte. Veuillez attendre la réinitialisation hebdomadaire.")
        }
      } else {
        console.log("trackDeviceLogin: Admin user, skipping device limit check")
      }

      // Insert new device with proper device name
      console.log("trackDeviceLogin: Inserting new device with name:", deviceName)
      ...
      if (insertError) {
        if (insertError.code === "23505") {
          console.log("trackDeviceLogin: Device already exists (race condition), continuing...")
        } else {
          console.error("trackDeviceLogin: Insert error:", insertError)
          throw insertError
        }
      } else {
        console.log("trackDeviceLogin: Device inserted successfully")
      }
    }
  } catch (error) {
    console.error("trackDeviceLogin: Error:", error)
    throw error
  }
}
...
async function updateUnknownDevices(userId) {
  console.log("updateUnknownDevices: Checking for devices with null device_name...")

  const client = await getSupabaseClient()
  if (!client) {
    console.error("updateUnknownDevices: Supabase client not available")
    return
  }
  ...
  try {
    ...
    if (fetchError) {
      console.error("updateUnknownDevices: Fetch error:", fetchError)
      return
    }

    if (!unknownDevices || unknownDevices.length === 0) {
      console.log("updateUnknownDevices: No unknown devices found")
      return
    }

    console.log("updateUnknownDevices: Found", unknownDevices.length, "unknown devices")
    ...
    for (const device of unknownDevices) {
      ...
      console.log("updateUnknownDevices: Updating device", device.id, "to", deviceName)
      ...
      if (updateError) {
        console.error("updateUnknownDevices: Update error for device", device.id, ":", updateError)
      } else {
        console.log("updateUnknownDevices: Successfully updated device", device.id)
      }
    }

    console.log("updateUnknownDevices: Update complete")
  } catch (error) {
    console.error("updateUnknownDevices: Error:", error)
  }
}
...
async function login(email, pin) {
  console.log("Login attempt:", { email, pinLength: pin?.length })
  ...
  console.log("All matches for email:", { count: allMatches?.length, matches: allMatches, searchError })
  ...
  if (error || !user) {
    console.error("Login failed:", { error, data, user })
    throw new Error("Email ou PIN incorrect")
  }

  console.log("login: Checking DeviceFingerprint availability...")
  console.log("login: window.DeviceFingerprint?", !!window.DeviceFingerprint)
  ...
  if (window.DeviceFingerprint && window.DeviceFingerprint.generate) {
    try {
      ...
      console.log("login: Generated fingerprint:", fingerprint)
      ...
      console.log("login: Current device found?", !!currentDevice)

      // If device limit reached and this is a new device, block login
      if (deviceCheck.isLimitReached && !currentDevice) {
        console.error("login: Device limit reached, blocking login")
        throw new Error("DEVICE_LIMIT_REACHED")
      }

      // Track this device login
      console.log("login: Calling trackDeviceLogin...")
      await trackDeviceLogin(user.id, fingerprint, deviceInfo)
      console.log("login: Device tracked successfully")
    } catch (deviceError) {
      console.error("login: Device tracking error:", deviceError)
      if (deviceError.message === "DEVICE_LIMIT_REACHED") {
        throw deviceError
      }
      // Don't block login for other device tracking errors
      console.warn("login: Continuing login despite device tracking error")
    }
  } else {
    console.warn("login: DeviceFingerprint not available, skipping device tracking")
  }
...
async function getSession() {
  ...
  return sessionStorage.getItem("fr_current_user")
}
...
async function deleteUser(userId) {
  ...
  console.log("deleteUser: Deleting user data...", { userId, email: userData?.email })

  // Delete from users table first
  const { error: deleteUserError } = await client.from("users").delete().eq("id", userId)

  if (deleteUserError) {
    console.error("deleteUser: Error deleting from users table:", deleteUserError)
  ...
  if (userId) {
    ...
    if (deleteDevicesError) {
      console.error("deleteUser: Error deleting device sessions:", deleteDevicesError)
    } else {
      console.log("deleteUser: Deleted all device sessions for user")
    }
  }
  ...
  if (userData?.email) {
    ...
    if (deleteRequestError) {
      ...
      console.log("deleteUser: Deleted registration requests for email")
    }
  }
  ...
  console.log("deleteUser: User deleted successfully. Note: Auth user may still exist in Supabase Auth.")
  console.log(
    "deleteUser: The login function now checks for pending requests to prevent orphaned auth users from logging in.",
  )
}
...
async function createAdminAccount(adminData) {
  ...
  console.log("Creating admin account:", { email: adminData.email, name: adminData.name })
  ...
  console.log("Admin account created successfully:", data.email)
  return data
}
...
async function getExerciseById(id) {
  console.log("getExerciseById: Looking for exercise with id:", id)
  const client = await getSupabaseClient()
  if (!client) throw new Error("Supabase not available")

  const { data, error } = await client.from("exercises").select("*").eq("id", id).single()

  console.log("getExerciseById: Query result - data:", data, "error:", error)
  if (error) throw error
  return data
}
...
async function createExercise(exercise) {
  ...
  const currentUser = JSON.parse(sessionStorage.getItem("fr_current_user"))

  // Generate a unique ID: type-titre (e.g., "lecture-1", "ecoute-5")
  const exerciseId = `${exercise.type}-${exercise.titre || exercise.title}`
...
async function getAttempts(userId = null) {
  // Placeholder for attempt retrieval logic
  return getUserAttempts(userId)
}

async function getExerciseHistory(exerciseId) {
  // Placeholder for exercise history retrieval logic
  const client = await getSupabaseClient()
  if (!client) throw new Error("Supabase not available")
...
async function getStudentHistory(userId) {
  // Placeholder for student history retrieval logic
  const client = await getSupabaseClient()
  if (!client) throw new Error("Supabase not available")
...
async function approveRegistration(requestId) {
  console.log("Starting approveRegistration with requestId:", requestId)
  const client = await getSupabaseClient()
  if (!client) throw new Error("Supabase not available")
  ...
  console.log("Marking request as approved with id:", requestId)
  const { error: updateError } = await client
    .from("registration_requests")
    .update({
      status: "approved",
  ...
  console.log("Registration approved successfully, status updated")
...
async function getRegistrationRequests() {
  // Placeholder for registration request retrieval logic
  return getAllRegistrations()
}

async function getProcessedRegistrations() {
  // Placeholder for processed registration retrieval logic
  const client = await getSupabaseClient()
  if (!client) throw new Error("Supabase not available")
...
async function getResources() {
  console.log("getResources: Starting to fetch external resources...")
  const client = await getSupabaseClient()
  if (!client) {
    console.error("getResources: Supabase client not available")
    throw new Error("Supabase not available")
  }

  console.log("getResources: Querying external_resources table...")
  const { data, error } = await client.from("external_resources").select("*").order("created_at", { ascending: false })

  if (error) {
    console.error("getResources: Error fetching resources:", error)
    throw error
  }

  console.log("getResources: Successfully fetched resources:", data?.length || 0, "items")
  return data || []
}
...
window.SupabaseDB = window.SupabaseDB || {
  ...
  getProcessedRegistrations: getProcessedRegistrations,
