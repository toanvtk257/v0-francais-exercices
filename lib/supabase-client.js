...
function getSupabaseClient() {
  if (!supabaseClient) {
    if (typeof window.supabase !== "undefined" && window.supabase.createClient) {
      ...
      console.log("[v0] Supabase client initialized:", !!supabaseClient)
    } else {
      console.error("[v0] Supabase library not loaded")
    }
  }
...
async function loginWithEmailAndPin(email, pin) {
  ...
  console.log("[v0] User logged in successfully:", data.email)
