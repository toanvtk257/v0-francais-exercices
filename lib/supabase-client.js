...
async function getClassMembers(classId) {
  ...
  try {
    // Call the simple RPC function that returns ONLY membership data
    const { data: members, error } = await client.rpc('get_class_members_simple', {
      p_class_id: classId
    })
    ...
    // Now fetch user data separately if we have members
    if (members && members.length > 0) {
      const userIds = members.map(m => m.user_id).filter(Boolean)
      
      if (userIds.length > 0) {
        // Fetch users using a simple table query without any relationships
        ...
        if (!usersError && users) {
          ...
          console.log("[v0] getClassMembers: Fetched", enriched.length, "members with user data")
          return enriched
        }
      }
    }

    console.log("[v0] getClassMembers: Fetched", members?.length || 0, "members")
    return members || []
  } catch (err) {
