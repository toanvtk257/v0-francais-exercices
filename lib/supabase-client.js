...
async function deleteUser(userId) {
  ...
  console.log("[v0] deleteUser called with userId:", userId)
  ...
  console.log("[v0] Deleted from users table, error:", deleteUserError)

  if (deleteUserError) throw deleteUserError

  // Also delete from registration_requests to allow email reuse
  if (userData?.email) {
    console.log("[v0] Attempting to delete from registration_requests for email:", userData.email)
    const { error: deleteRequestError } = await client
      .from("registration_requests")
      .delete()
      .eq("email", userData.email)

    console.log("[v0] Deleted from registration_requests, error:", deleteRequestError)
