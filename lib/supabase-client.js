...
async function loginWithEmailAndPin(email, pin) {
  ...
  if (window.DeviceFingerprint) {
    try {
      const deviceFingerprint = window.DeviceFingerprint.getFingerprint()
      const deviceName = window.DeviceFingerprint.getDeviceName()
      ...
      const currentDevice = devices.find((d) => d.device_fingerprint === deviceFingerprint)
      ...
      await trackDeviceLogin(user.id, deviceFingerprint, deviceName)
    } catch (deviceError) {
...
async function trackDeviceLogin(userId, deviceFingerprint, deviceName) {
  const client = await getSupabaseClient()
  if (!client) throw new Error("Supabase not available")
  ...
  if (checkError && checkError.code !== "PGRST116") {
    console.error("[v0] Error checking device:", checkError)
  }
  ...
  if (existing) {
    ...
    const { data, error } = await client
      ...
      .update({
        ...
        is_trusted: existing.login_count + 1 >= 3, // Trust after 3+ logins
      })
      .eq("id", existing.id)
      .select()
      .single()
...
async function deleteDevice(deviceId) {
  ...
  const { error } = await client.from("device_sessions").delete().eq("id", deviceId)
...
async function checkDeviceLimit(userId) {
  ...
  // Count devices used in the last 7 days
  const oneWeekAgo = new Date()
  oneWeekAgo.setDate(oneWeekAgo.getDate() - 7)

  const recentDevices = devices.filter((d) => new Date(d.last_login) > oneWeekAgo)

  return {
    deviceCount: recentDevices.length,
    maxDevices: MAX_DEVICES,
    isLimitReached: recentDevices.length >= MAX_DEVICES,
    devices: recentDevices,
  }
}

async function canRemoveDevice(deviceId) {
  const client = await getSupabaseClient()
  if (!client) throw new Error("Supabase not available")

  const { data, error } = await client.from("device_sessions").select("created_at").eq("id", deviceId).single()
  ...
  return {
    canRemove: daysSinceCreation >= 7,
    daysRemaining: Math.max(0, 7 - daysSinceCreation),
    unlockDate: new Date(createdDate.getTime() + 7 * 24 * 60 * 60 * 1000),
  }
}
...
window.SupabaseDB = {
  ...
  canRemoveDevice: canRemoveDevice, // Added canRemoveDevice to exports
}
