...
async function deleteUser(userId) {
  ...
  // Get user email before deletion to allow email reuse
  ...
  // Also delete from registration_requests to allow email reuse
  if (userData?.email) {
    ...
    if (deleteRequestError) console.error("Error deleting registration request:", deleteRequestError)
  }
}
...
async function approveRegistration(requestId) {
  console.log("[v0] approveRegistration called with requestId:", requestId)
  const client = await getSupabaseClient()
  if (!client) throw new Error("Supabase not available")
  ...
  console.log("[v0] Fetched registration request:", request, "Error:", fetchError)
  if (fetchError) throw fetchError
  ...
  console.log("[v0] Created user:", newUser, "Error:", createError)
  if (createError) throw createError

  console.log("[v0] Attempting to delete registration_request with id:", requestId)
  ...
  if (deleteError) {
    console.error("[v0] Delete failed with error:", deleteError)
    throw deleteError
  }

  console.log("[v0] Registration approved successfully, request deleted")
