...
async function getClassMembers(classId) {
  ...
  try {
    // Use raw SQL to completely bypass Supabase's relationship auto-detection
    const { data, error } = await client.rpc('get_class_members', {
      p_class_id: classId
    })

    if (error) {
      console.error("[v0] getClassMembers: Error with RPC:", error)
      // Fallback to direct query if RPC doesn't exist
      const { data: members, error: queryError } = await client
        .from("class_memberships")
        .select("id, user_id, created_at, added_by, class_id")
        .eq("class_id", classId)
        .order("created_at", { ascending: false })

      if (queryError) {
        console.error("[v0] getClassMembers: Error fetching members:", queryError)
        throw queryError
      }

      console.log("[v0] getClassMembers: Fetched", members?.length || 0, "members (fallback)")
      return members || []
    }

    console.log("[v0] getClassMembers: Fetched", data?.length || 0, "members")
    return data || []
  } catch (err) {
