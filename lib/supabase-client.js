...
async function getClassMembers(classId) {
  ...
  try {
    // Query from the view - no embedded relationships, so no PGRST201 error
    ...
    if (members && members.length > 0) {
      ...
      if (userIds.length > 0) {
        ...
        if (usersError) {
          console.error("[v0] getClassMembers: Error fetching users:", usersError)
          // Don't throw - continue with members data even if user fetch fails
        }
        ...
        console.log("[v0] getClassMembers: Fetched", enrichedMembers.length, "members with user data")
        return enrichedMembers
      }
    }

    console.log("[v0] getClassMembers: Fetched", members?.length || 0, "members (no users)")
    return members || []
  } catch (err) {
