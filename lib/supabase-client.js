...
async function getClassMembers(classId) {
  console.log("[v0] getClassMembers: Fetching members for class:", classId)
  const client = await getSupabaseClient()
  if (!client) throw new Error("Supabase not available")

  try {
    // Call the flattened RPC function that already has user data joined at the SQL level
    // This completely bypasses PostgREST's relationship embedding by returning flat data
    const { data: flatMembers, error } = await client.rpc('get_class_members_with_users', {
      p_class_id: classId
    })

    if (error) {
      console.error("[v0] getClassMembers: Error fetching members:", error)
      throw error
    }

    // Transform flat data back into the expected format with nested users object
    const members = flatMembers?.map(row => ({
      id: row.member_id,
      class_id: classId,
      user_id: row.user_id,
    ...
    console.log("[v0] getClassMembers: Fetched", members.length, "members with user data")
    return members
  } catch (err) {
