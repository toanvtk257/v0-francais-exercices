...
async function approveRegistration(requestId) {
  ...
  // This allows the email to be reused if the account is later deleted
  const { error: deleteError } = await client.from("registration_requests").delete().eq("id", requestId)
